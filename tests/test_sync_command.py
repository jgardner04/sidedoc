...
def test_sync_command_detects_changes() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with zipfile.ZipFile(sidedoc_path, "a") as zf:
            # Remove old content.md and add new one
            pass  # Can't remove in append mode, but overwrite works

        # Rewrite with new content
        with zipfile.ZipFile(sidedoc_path, "r") as zf_read:
            styles = zf_read.read("styles.json")
            manifest = zf_read.read("manifest.json")
            structure = zf_read.read("structure.json")

        with zipfile.ZipFile(sidedoc_path, "w") as zf_write:
            zf_write.writestr("content.md", new_content)
            zf_write.writestr("styles.json", styles)
            zf_write.writestr("manifest.json", manifest)
            zf_write.writestr("structure.json", structure)
        ...
        with zipfile.ZipFile(sidedoc_path, "r") as zf:
            updated_structure = json.loads(zf.read("structure.json"))
...
def test_sync_command_invalid_sidedoc() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with zipfile.ZipFile(invalid_path, "w") as zf:
            zf.writestr("random.txt", "not a sidedoc")
