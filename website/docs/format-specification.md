...
Sidedoc uses a `.sidedoc/` directory as its canonical working format, containing structured data that enables round-trip document conversion between AI-friendly markdown and human-friendly formatted Word documents. A `.sdoc` ZIP archive provides a single-file distribution format. Both contain the same files.

## Container Structure

```
document.sidedoc/              (working format — default)
...
document.sdoc                  (distribution format — with --pack)
└── (same files as above, in a ZIP archive)
```

`structure.json` and `manifest.json` are optional in directories (generated by `sidedoc sync`) but required in `.sdoc` ZIPs.
...
| Tables | GFM pipe syntax | Table element | Merged cells, cell formatting, alignment |
| Track changes | CriticMarkup | Revisions | Insertions/deletions with author/date |
